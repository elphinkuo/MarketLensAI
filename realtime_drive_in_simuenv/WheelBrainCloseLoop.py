...
# Define a driver agent based on SambaNova_Llama_Vision

# ----------------------------------------------------------------------
class SambaNovaVisionAgent:
    def __init__(self, max_tokens: int = 4000) -> None:
        self.api_key = '57749b88-c7a1-4eb2-949f-a81d6cd6b892'
    ...
    def makeDecision(self):
        ...
        """
        print("Calling int makeDecision function")
        start = time.time()
        ...
        if match:
            ...
            raise ValueError('SambaNova_Llama_Vision did not return a valid decision')
        return (
...
if __name__ == '__main__':
    ...
    # init SambaNova_Llama_Vision-based driver agent
    sambanova_v = SambaNovaVisionAgent()
            ...
            if model.timeStep % 10 == 0:
                ...
                if model.tpStart and roadgraph:
                    ...
                    if isinstance(front_img, np.ndarray):
                        ...
                        sambanova_v.addTextPrompt(SYSTEM_PROMPT)
                        sambanova_v.addTextPrompt('The next three images are images captured by the left front, front, and right front cameras.\n')
                        sambanova_v.addImageBase64(NPImageEncode(front_left_img))
                        sambanova_v.addImageBase64(NPImageEncode(front_img))
                        sambanova_v.addImageBase64(NPImageEncode(front_right_img))
                        sambanova_v.addTextPrompt(f'\nThe current frame information is:\n{TotalInfo}')
                        sambanova_v.addTextPrompt('Now, please tell me your answer. Please think step by step and make sure it is right.')
                        # get the decision made by the driver agent
                        (
                            behaviour, ans, 
                            prompt_tokens, completion_tokens, 
                            total_tokens, timecost
                        ) = sambanova_v.makeDecision()
